...
HRESULT LightPass::createConstantBuffer()
{
	...
	hr = gDevice->CreateBuffer(&lbDesc, &lbData,
...
HRESULT LightPass::createShaders()
{
	...
	this->gDevice->CreateInputLayout(inputDescLight, ARRAYSIZE(inputDescLight), pVSL->GetBufferPointer(), pVSL->GetBufferSize(), &this->gVertexLayout);
	...
	this->gDevice->CreatePixelShader(pPSL->GetBufferPointer(), pPSL->GetBufferSize(), nullptr, &this->gPixelShader);
	// we do not need anymore this COM object, so we release it.
	pPSL->Release();
...
void LightPass::render()
{
	...
	this->gDeviceContext->IASetVertexBuffers(0, 1, &this->gLightBuffer, &vertexSizeLight, &offsetLight);
	...
	this->gDeviceContext->PSSetShaderResources(0, 1,
	...
	this->gDeviceContext->Draw(1000, 0);
}
